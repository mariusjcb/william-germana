...
function pickDirection(pref: 'de-en' | 'en-de' | 'mixed'): CardDirection {
  if (pref === 'mixed') return Math.random() < 0.5 ? 'de-en' : 'en-de';
  return pref;
}
...
function vocabToFlashcard(word: VocabWord, dir: CardDirection): FlashcardItem {
  return {
    id: word.id,
    front: dir === 'de-en' ? word.german : word.english,
    back: dir === 'de-en' ? word.english : word.german,
    direction: dir,
    meta: {
